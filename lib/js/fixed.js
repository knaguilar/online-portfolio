...
contactInfo.display = function() {
	$(".contact-wrapper > div > .contact-info").append("<p>Let's connect!</p><p>Find me here: </p>");
	$(".contact-wrapper > div > .contact-info").append('<ul></ul');
	for (var i = 0; i < contactInfo.links.length; i++) {
		$('<a href="'+contactInfo.links[i] +'"><i class="'+ contactInfo.social[i]+'" aria-hidden="true"></i></a>').appendTo(".contact-wrapper > div > .contact-info > ul");
	}
